...
export function ObjectDetector(props) {
 ...
 const openfilePicker =  () => {
  ...
  const detectObjectImage = async (imageElement, imgSize) => {
    ...
    // Thay đổi số lượng đối tượng cần phát hiện (đặt thành một số lớn hơn 10)
    const predictions = await model.detect(imageElement, { maxDetected: 20 });
    const normalizedPredictions = normalizePredictions(predictions, imgSize);
    setpredictions(normalizedPredictions);
    console.log("xin chào: ", predictions, imgSize);
  };
 ...
 return (
  <ObjectDetectorContainer>
    <DetectorContainer>
      <TargetImg src={imgData} ref={imageRef} hidden={!imgData} />
      {!isEmptyPredictions &&
